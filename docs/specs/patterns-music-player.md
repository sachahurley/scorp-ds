...
| Field | Value |
|-------|-------|
| Pattern | `Patterns/MusicPlayer` (story-only, not an exported component) |
| Layer | `pattern` |
| Category | `Patterns` |
...
| `Button` | `variant="icon"` for transport, close, collapse and expand; `variant="primary"` for play/pause; card transport at `size="md"` (40px), collapsed-bar transport and both mode toggles at `size="sm"` (32px); `variant="outline" size="sm"` "Reopen player" in the story shell. |
| `TuiIcon` | `Shuffle`, `SkipBack`, `Play`/`Pause`, `SkipForward`, `Repeat`, `Music2` (album placeholder at size 8, bar thumb, current-row marker at size 3), `X` (close), `ChevronDown` (collapse), `ChevronUp` (expand). |
| `ListRow` | Queue rows in the `WithQueue` story (`onClick` rows, meta "Track n · m:ss"). |
| `Divider`, `Stack` | Story shell footnote divider; `Stack gap="1"` for the queue. |
| `cn` | Class merging in the story-local `MarqueeText`, `DragHandle` and card. |
...
- **Timeline:** a native `input[type=range]` (click, drag, arrow keys for free). Fill is a two-stop gradient over theme-scoped custom props (`--scrub-fill`/`--scrub-track` mapped to `secondary-600/200`, dark `400/800`); the thumb is a sharp `--text-primary` nub; focus uses the `--focus-ring-*` tokens. The DS now ships a `Slider` component (`Components/Inputs/Slider`), but the timeline has not been migrated to it.
- **Transport hierarchy:** the expanded card's transport controls are uniform 40px `md` plates (Sacha's call, 2026-09-19; the collapsed bar dropped to 32px `sm` on 2026-09-20, see Collapsed bar); play/pause carries the emphasis through its primary fill alone. Known deviation: 40px sits under the DS's 44px touch-target law; the 48px play was the only compliant control and was traded for the uniform row. Shuffle/repeat expose `aria-pressed`, and the pressed style is driven off that attribute with `aria-pressed:` utilities (inverted plate fill, `secondary-800/50` light and `secondary-300/950` dark, with explicit pressed-hover steps). Do not mark toggles by swapping to the `secondary` variant: in dark mode `secondary` and `icon` resolve to identical resting colors, and `secondary`'s gold hover reads as a glitch on click.
...
- **Artist line:** subtle tone, `text-secondary-700 dark:text-secondary-500`. This was chosen when the semantic `--text-secondary` light value failed AA on the card surface; `--text-secondary` has since moved to sepia-700 (light) and is sepia-500 (dark), so the scale steps now equal the semantic token in both themes and could be swapped for it.
...
- The current queue row combines three signals: muted fill, a glyph suffix, and `aria-current="true"`. The fill is applied through `className`, not ListRow's `selected` prop.
...
|------|-------|------------|--------|
| 2026-09-22 | The timeline is a hand-rolled range input although `Slider` now exists; the queue marks the current row with a `className` fill instead of ListRow's `selected`; the elevation drop-shadow is an inline `rgba` value | None yet | open |
